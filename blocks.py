            ...
            for j,sub_sub_block in enumerate(m.layer):
                if index == i:
                    if (basic_or_bottle == BottleBlock):
                        if mask:
                            m.layer[j] = MaskBottleBlock(sub_sub_block.in_channels, sub_sub_block.out_channels, conv=new_conv, stride=sub_sub_block.stride)
                        else:
                            m.layer[j] = BottleBlock(sub_sub_block.in_channels, sub_sub_block.out_channels, conv=new_conv, stride=sub_sub_block.stride)
                    else:
                        if mask:
                            m.layer[j] = MaskBlock(sub_sub_block.in_channels, sub_sub_block.out_channels, conv=new_conv, stride=sub_sub_block.stride)
                        else:
                            m.layer[j] = BasicBlock(sub_sub_block.in_channels, sub_sub_block.out_channels, conv=new_conv, stride=sub_sub_block.stride)
                i = i + 1
